...
import { Float, Sphere, Box, Torus, Environment } from '@react-three/drei'
...
function FloatingBubble({ position, color }: { position: [number, number, number]; color: string }) {
  const meshRef = useRef<THREE.Mesh>(null)
  ...
  return (
    <mesh ref={meshRef} position={position}>
      <sphereGeometry args={[0.2 + Math.random() * 0.15, 32, 32]} />
...
function Scene() {
  ...
  return (
    <>
      ...
      {/* Floating bubbles */}
      <FloatingBubble position={[-3.5, -0.5, 0.5]} color="#FF6B9D" />
      <FloatingBubble position={[3.5, 1, -0.5]} color="#FFD93D" />
      <FloatingBubble position={[0, -2, 0]} color="#C77DFF" />
      <FloatingBubble position={[-2, 0, 1]} color="#4FC3F7" />
      <FloatingBubble position={[2, 2, 0.5]} color="#6BCB77" />
      <FloatingBubble position={[1, -1, -1]} color="#FF8B64" />
